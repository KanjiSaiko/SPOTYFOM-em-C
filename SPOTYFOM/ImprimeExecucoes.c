...
void desenhaMusicasRandom(float largura, float altura, struct TelaExec *TelaMusicas, DescFila *FilaCorreta){
    ...
    if(FilaCorreta->head == NULL){
        mvwprintw(TelaMusicas->content, altura/3, (largura-strlen("Playlist Vazia"))/2, "Playlist Vazia");
    }
    while (aux != NULL) {
        //26 musicas por tela
...
void inicializa_ncurses() {
    ...
    keypad(stdscr, TRUE);
    curs_set(0);
    mousemask(ALL_MOUSE_EVENTS, NULL); // Habilita captura de eventos do mouse
}
